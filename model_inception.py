...
class Encoder(nn.Module):
    ...
    def forward(self, x_in):
        ...
        return self.ds4_out, self.fc1_out
...
class Dis(nn.Module):
    ...
    def forward(self, x_in):
        self.fea, self_vec = self.enc(x_in)
        self.result = self.fc(self.fea)
        return self.fea, self_vec, self.result
...
class Gen(nn.Module):
    ...
    def infer_ER(self, x_ER):
        fea_ER,_ = self.enc_ER(x_ER)
        self.enc_FR
        result_ER = self.fc_ER(fea_ER)
        return fea_ER,_, result_ER
    ...
    def gen_img_withfea(self, fea_FR, fea_ER, noise=None, device=None):
        self.enc_FR.ds4_out = fea_FR
        self.enc_ER.ds4_out = fea_ER
        self.img = self.dec(enc_FR=self.enc_FR, enc_ER=self.enc_ER, noise=noise, device=device)
        return self.img

    def gen_img_ipd(self, fea_FR, x_ER, noise=None, device=None):
        self.enc_FR.ds4_out = fea_FR
        self.fea_ER = self.infer_ER(x_ER=x_ER)
        self.img = self.dec(enc_FR=self.enc_FR, enc_ER=self.enc_ER, noise=noise, device=device)
        return self.img
